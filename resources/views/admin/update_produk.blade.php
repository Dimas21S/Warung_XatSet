...
<head>
    ...
    <title>Edit Produk - Warung Xat Set</title>
    <style>
...
<div class="flex">
    ...
    <div class="flex-1" style="margin-left: 220px;">
        ...
        <div class="p-6">
            ...
            {{-- Form --}}
            <form action="{{ route('admin.produk.update', $menu->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" class="input-field" rows="3"
                                  placeholder="Deskripsi produk...">{{ old('deskripsi', $menu->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            <select name="outlet_id" class="input-field">
                                ...
                                @foreach($outlets as $outlet)
                                    <option value="{{ $outlet->id }}"
                                        {{ old('outlet_id', $menu->outlet_id) == $outlet->id ? 'selected' : '' }}>
                                        {{ $outlet->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center cursor-pointer hover:border-green-500 transition"
                            ...
                            @if($menu->gambar)
                                <img id="preview-gambar"
                                     ...
                                     class="w-40 h-40 object-cover rounded-xl mx-auto mb-3">
                                <p id="placeholder-gambar" class="text-gray-400 text-xs">Klik untuk ganti gambar</p>
                            @else
